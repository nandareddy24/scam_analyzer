...
class URLPhishingModel:
    """
    Independent scikit-learn Machine Learning pipeline for URL phishing classification.
    ...
    def __init__(self, filepath: Optional[str] = None):
        ...
        default_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "models", "url_model.joblib"))
    ...
    def fit(self, X_train: List[str], y_train: List[int]):
        processed_X = [self.preprocess_url(x) for x in X_train]
        self.pipeline.fit(processed_X, y_train)

    def predict(self, X: List[str]) -> List[int]:
        processed_X = [self.preprocess_url(x) for x in X]
    ...
    def evaluate(self, X_test: List[str], y_test: List[int]) -> Dict[str, Any]:
        ...
        return {
            "model_name": "URLPhishingModel (Char TF-IDF + RandomForest)",
